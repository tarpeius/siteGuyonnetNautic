<header>
<!-- MENU -->
    <nav>
        <div class="nav-wrapper grey darken-3">
<!--            <div>-->
<!--                <a href="#!" class="brand-logo"><img id="logoMenu" src="Util/img/Copie de GUYONNET-logo.png"></a>-->
<!--            </div>-->
            <ul class="left hide-on-med-and-down">
                <li id="homeNav"><a class="dropdown-button" href="#!"> <i class="material-icons">home</i> </a></li>
                <li class="CategNav"><a class="dropdown-button" href="#!" data-activates="dropdown1">Bateau</a></li>
                <li class="CategNav"><a class="dropdown-button" href="#!" data-activates="dropdown2">Jet</a></li>
                <li class="CategNav"><a class="dropdown-button" href="#!" data-activates="dropdown3">Moteur</a></li>
                <li class="CategNav"><a class="dropdown-button" href="#!" data-activates="dropdown4">Remorque</a></li>
                <li class="CategNav"><a class="dropdown-button" href="#!" data-activates="dropdown5">Armement</a></li>
                <li class="CategNav"><a class="dropdown-button" href="#!" data-activates="dropdown6">Permis</a></li>
                <li class="CategNav"><a class="dropdown-button" href="#!" data-activates="dropdown7">Nos Services</a></li>
            </ul>
            <ul class="right hide-on-med-and-down">
                <li class=""><a class="dropdown-button" href="#!" data-activates="dropdown8">Contact</a>
                <li id="homeNav"><a class="dropdown-button" href="#!" data-activates="dropdown8"> <i class="large material-icons">account_circle</i> </a>
            </ul>
        </div>
    </nav>
    <!-- Dropdown Structure -->
    <ul id="dropdown1" class="dropdown-content">
        <li class="SousCategNav"><a href="#!">Pacific Craft</a></li>
        <li class="divider"></li>
        <li class="SousCategNav"><a href="#!">Zodiac</a></li>
        <li class="divider"></li>
        <li class="SousCategNav"><a href="#!">Capelli Tempest</a></li>
        <li class="divider"></li>
        <li class="SousCategNav"><a href="#!">Rigiflex</a></li>
        <li class="divider"></li>
        <li class="SousCategNav"><a href="#!">Fun-Yack</a></li>
    </ul>
    <ul id="dropdown2" class="dropdown-content">
        <li class="SousCategNav"><a href="#!">Seadoo</a></li>
    </ul>
    <ul id="dropdown3" class="dropdown-content">
        <li class="SousCategNav"><a href="#!">Yamaha</a></li>
    </ul>
    <ul id="dropdown4" class="dropdown-content">
        <li class="SousCategNav"><a href="#!">Rocca</a></li>
    </ul>
    <ul id="dropdown5" class="dropdown-content">
        <li class="SousCategNav"><a href="#!">Plastimo</a></li>
        <li class="divider"></li>
        <li class="SousCategNav"><a href="#!">Navicom</a></li>
    </ul>
    <ul id="dropdown6" class="dropdown-content">
        <li class="SousCategNav"><a href="#!">Cotier</a></li>
        <li class="divider"></li>
        <li class="SousCategNav"><a href="#!">Fluvial</a></li>
        <li class="divider"></li>
        <li class="SousCategNav"><a href="#!">Hauturier</a></li>
    </ul>
<!-- PHOTO CATEGORIE BATEAU -->
    <div id="rowCateg" class="row">
        <div class="col m2 s12 divPhoto">
            <a href=""> <img class="imgCateg" src="Util/img/4-navigation-3-625-5ecab4ac55.jpg">
                <div class="blocTitle">
                    <div class="title">Bateau</div>
                    <div class="button">
                        <span class="btn waves-effect waves-light grey darken-3">Voir</span>
                    </div>
                </div>
            </a>
        </div>
<!-- PHOTO CATEGORIE BATEAU -->
<!-- PHOTO CATEGORIE JET -->
        <div class="col m2 divPhoto">
            <a href=""> <img class="imgCateg" src="Util/img/RXT-X 300_332_MY18.jpg">
                <div class="blocTitle">
                    <div class="title">Jet</div>
                    <div class="button">
                        <span class="btn waves-effect waves-light grey darken-3">Voir</span>
                    </div>
                </div>
            </a>
        </div>
<!-- PHOTO CATEGORIE JET -->
        <div class="col m2 divPhoto">
            <a href=""> <img class="imgCateg" src="Util/img/2017-Yamaha-F130-EU-NA-Static-002.jpg">
                <div class="blocTitle">
                    <div class="title">Moteur</div>
                    <div class="button">
                        <span class="btn waves-effect waves-light grey darken-3">Voir</span>
                    </div>
                </div>
            </a>
        </div>
        <div class="col m2 divPhoto">
            <a href=""> <img class="imgCateg" src="Util/img/GTR 230_191_MY17.jpg">
                <div class="blocTitle">
                    <div class="title">Remorque</div>
                    <div class="button">
                        <span class="btn waves-effect waves-light grey darken-3">Voir</span>
                    </div>
                </div>
            </a>
        </div>
        <div class="col m2 divPhoto">
            <a href=""> <img class="imgCateg" src="Util/img/autre nom - GTR 230_1738_MY17.jpg">
                <div class="blocTitle">
                    <div class="title">Armement</div>
                    <div class="button">
                        <span class="btn waves-effect waves-light grey darken-3">Voir</span>
                    </div>
                </div>
            </a>
        </div>
        <div class="col m2 divPhoto">
            <a href=""> <img class="imgCateg" src="Util/img/GTX LTD - RXT-X large_2J8A6789_MY18.jpg">
                <div class="blocTitle">
                    <div class="title">Permis</div>
                    <div class="button">
                        <span class="btn waves-effect waves-light grey darken-3">Voir</span>
                    </div>
                </div>
            </a>
        </div>
    </div>
</header>
<script>

    $('.dropdown-button').dropdown({
            inDuration: 300,
            outDuration: 225,
            constrainWidth: false, // Does not change width of dropdown to that of the activator
            hover: true, // Activate on hover
            gutter: 0, // Spacing from edge
            belowOrigin: false, // Displays dropdown below the button
            alignment: 'left', // Displays dropdown with edge aligned to the left of button
            stopPropagation: false // Stops event propagation
        }
    );
</script>